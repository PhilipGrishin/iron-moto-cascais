...
- Verified: The approved Wave 4 source checksum, 80-page visible-content
  boundary, exact 80-URL sitemap delta, missing-mapping break test, focused
  Wave 4 validator and all existing validator families passed. A clean-clone
  Full Safe Rebuild was idempotent. Production workflow `34156212818`, all 80
  production URLs, both responsive breakpoints and two Google Rich Results
  reports passed; full evidence is in `docs/reports/S_REBUILD_W4_REPORT.md`.
- Notes: The five Markdown-backed posts receive the Wave 4 `modifiedISO` only
  ...
  sources remain unchanged and only their visible-text pins moved. The same
  Blog post-load step now guarantees that author/publisher `#business`
  references resolve to a complete `LocalBusiness` node.
