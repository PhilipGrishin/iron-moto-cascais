...
Its `committed` job output conditionally calls the reusable Pages workflow. A
no-change refresh skips that call entirely, so it neither creates a Pages run
nor interrupts an in-progress deployment.
