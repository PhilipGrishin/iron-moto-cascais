...
- Brand service pages are registered in
  ...
  `PAGE_I18N` content for all 4 languages. Same flow: edit data,
  run `build_brand_pages.py`, then run `validate_brand_pages.py`
  for the new slug.
...
### Add or update the Authorized Dealer hub

1. Edit hub copy, metadata, FAQ and future dealer-brand cards in
   `scripts/build/authorized_dealer_data.py`.
2. If the hero image changes, replace
   `photos/authorized-dealer-main-1600.jpg` and run
...
4. When adding a root-level generated folder, make sure
   `.github/workflows/pages.yml` copies it into the GitHub Pages
   artifact.

### Add a brand service page

1. Add a `BRAND_CONFIG` entry and `BRAND_ORDER` position in
   `scripts/build/brand_pages_data.py`.
2. Add `BRAND_HEAD` and `PAGE_I18N` for EN/RU/UK/PT using the
   existing BMW/Harley/Ducati/Suzuki structure.
3. Add the global nav label in `assets/main.js` and run
   `node scripts/build/extract_i18n.js`.
4. Add the hero source photo under `photos/`, run
